...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["receiver_id"])) {
    ...
    $sql = "SELECT status FROM friend_requests WHERE (sender_id = ? AND receiver_id = ?) OR (sender_id = ? AND receiver_id = ?)";
    ...
    if ($status) {
        // Nếu đã có lời mời hoặc đã là bạn bè thì không gửi lại
